const createButtonStyle = (isDisabled) => {
  const buttonTypeStyle = {
    ...
    mediumSquareBlack: `w-[180px] h-[42px] bg-blackSecondary border-whiteSecondary-500 border rounded-[3px] hover:bg-blackHover transition duration-500 ${!isDisabled ? '' : 'cursor-not-allowed bg-grayMedium'}`,
    mediumSquarePrimary: `w-[180px] h-[42px] rounded-[3px] ${!isDisabled ? 'transition-all duration-500 bg-pos-0 hover:bg-pos-100 bg-size-200 bg-gradient-to-r from-pointOrange via-pointPink to-pointOrange hover:scale-105' : 'cursor-not-allowed bg-grayMedium'}`,
  };
